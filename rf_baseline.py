"""
Random Forest Baseline for DSAA 2026 Paper
...
ASSETS = {
    "AAPL": PROJECT_ROOT / "dataset" / "AAPL_D_2004_2023.csv",
    "Gold": PROJECT_ROOT / "dataset" / "GOLD_D_2004_2023.csv",
}
...
OUTPUT_RESULTS_DIR = PROJECT_ROOT / "results" / "rf"
...
def run_all():
    ...
    logging.info("Starting Random Forest Baseline Backtest Engine...")
    ...
    for asset_name, data_path in ASSETS.items():
        ...
        for period_name, window in STRESS_WINDOWS.items():
            ...
            # Random Forest Classifier with identical parameters
            ...
            for scen_name, scen_cfg in SCENARIOS.items():
                ...
                run_id = f"{asset_name.upper()}_RF_{period_name.replace('-', '_')}_{scen_name}"
                ...
                results_json = {
                    ...
                    "llm": {
                        ...
                        "system_prompt": {
                            ...
                            "file": None,
                            "override": False,
                            "preview": "Random Forest Classifier + 1% Risk Gating",
                            "full": "Random Forest classifier trained on walk-forward historical features. Order execution guided by volatility gating and stop loss gates."
                        }
                    },
                    "trading": {
    ...
    if all_feature_importances:
        ...
        fi_df.to_csv(OUTPUT_RESULTS_DIR / "rf_feature_importance.csv", index=False)
        logging.info(f"Saved feature importance -> {OUTPUT_RESULTS_DIR}/rf_feature_importance.csv")
        ...
        try:
            ...
            plt.title("Random Forest Average Feature Importance (MDI)", fontsize=14, fontweight="bold")
            plt.xlabel("Relative Importance (MDI)")
            plt.tight_layout()
            plt.savefig(OUTPUT_RESULTS_DIR / "rf_feature_importance.png", dpi=300)
            plt.close()
            ...
            plt.title("Random Forest Average Feature Importance by Group", fontsize=14, fontweight="bold")
            plt.xlabel("Total Group Importance")
            plt.tight_layout()
            plt.savefig(OUTPUT_RESULTS_DIR / "rf_group_importance.png", dpi=300)
            plt.close()
            
            # SHAP Summary Beeswarm Plot
            plt.figure(figsize=(10, 6))
            shap.summary_plot(shap_vals_all, X_test_all, show=False)
            plt.title("SHAP Summary Beeswarm Plot - Random Forest (All Assets & Periods)", fontsize=14, fontweight="bold")
            plt.tight_layout()
