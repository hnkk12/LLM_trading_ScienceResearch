"""
XGBoost Baseline for DSAA 2026 Paper
...
RISK_CONFIG = {
    ...
    "initial_capital":     1000.0, # 1,000 USD to match Baseline/RMDB/Llama
    "fee_rate_taker":      0.0005, # 0.05% taker fee
}

OUTPUT_RESULTS_DIR = PROJECT_ROOT / "results" / "xgboost"
...
def run_all():
    ...
    logging.info("Starting XGBoost Baseline Backtest Engine...")
    ...
    for asset_name, data_path in ASSETS.items():
        ...
        for period_name, window in STRESS_WINDOWS.items():
            ...
            for scen_name, scen_cfg in SCENARIOS.items():
                ...
                # This ensures direct compatibility with significance testing tools
                ...
                # Daily return format required by paper specifications
    ...
    # Average across period/asset for diagnostics summary
...
def generate_mdd_advantage_counts(xgb_agg_df: pd.DataFrame):
    ...
    # Combine everything to results/table2_combined.csv if other runs are available
    all_perf = pd.concat([others_df, xgb_agg_df], ignore_index=True)
    combined_table_path = PROJECT_ROOT / "results" / "table2_combined.csv"
